...
def parse_ord(reader,pages):
    ...
    full_text = '\n'.join(reader.pages[page_no].extract_text() for page_no in pages).splitlines()
    ...
    for line in full_text:
        if q_match := question_pattern.match(line):
            ...
            current_question_number_1, current_question_text_1, current_question_number_2, current_question_text_2 = map(str.strip, q_match.groups())
...
def parse_mek(reader, pages):
    ...
    for page_num in pages:  # page number indexing starts from 0
        ...
        text_box = (0, 0, page.width, page.height - 50)  # A box excluding the bottom 50 points of the page
        ...
        for line in lines:
            ...
            if match:
                ...
                answer_options = {}  # Reset answer options as a dict now (e.g., {'a': '...', 'b': '...'})
            elif re.match(r'^[A-D]\s+', line):  # If the line starts with A, B, C, or D (indicating an answer option)
                option_letter = line[0].lower()  # Convert 'A', 'B', 'C', 'D' to 'a', 'b', 'c', 'd'
                option_text = re.sub(r'\s+', ' ', line[2:])  # Extract the text after the letter and space
